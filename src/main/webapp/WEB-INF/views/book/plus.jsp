			...
			<div id="content_head">
				...
				<div class="head_right">
					...
					<div class="author-info">
						...
						<c:choose>
							<c:when test="${detailDto.author_num==detailDto.illu_num}">
								<span class="author illustrator"><a href="${root}/author.do?${detailDto.author_num}">{detailDto.author_name}</a></span> 
							</c:when>
							
							<c:otherwise>
								<c:if test="${detailDto.author_num!=0}">
									<span class="author "><a href="${root}/author.do?${detailDto.author_num}">{detailDto.author_name}</a></span> 
								</c:if>
								<c:if test="${detailDto.illu_num!=0}">
									<span class="illustrator "><a href="${root}/author.do?${detailDto.illu_num}">{detailDto.illu_name}</a></span>
								</c:if>
							</c:otherwise>
						</c:choose>
						
						<c:if test="${detailDto.trans_num!=0}">
							<span class="translator "><a href="${root}/author.do?${detailDto.trans_num}">{detailDto.trans_name}</a></span>
						</c:if>
					</div>
								...
								<c:choose>
									<c:when test="${detailDto.book_price!=0}">
										...
										<c:if test="${detailDto.discount2!=0 && detailDto.discount!=0}">								
											<div>
												<c:set var="sailPrice2" value="${sail*(1-(detailDto.discount2*0.01))}"/>
												<fmt:parseNumber var="sail2" value="${sailPrice}" integerOnly="true"/>
										...
										<c:if test="${detailDto.discount2!=0 && detailDto.discount==0}">
											<div>
												<span>판매가</span> <span class="sell">${sail}
												<span class="sail"></span></span>
											</div>
										</c:if>
