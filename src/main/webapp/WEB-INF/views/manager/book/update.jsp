...
<body>
		...
		<form id="bookInsertForm" action="${root}/manager/bookUpdateOk.do" method="post" onsubmit="return bookUpdateOk()">
			<section class="b_in_main">
				...
				<div class="b_in_content">
					<div>
						<label>도서 제목</label>
						<input type="text" placeholder="제목" value="${bookDto.name}" oninput="checkName('${root}')" disabled="disabled"/>
						<input type="hidden" name="name" value="${bookDto.name}"/>
						<span class="b_in_check"></span>
					</div>
					<div>
						<label>도서 부제목</label>
						<input type="text" name="sub_title" placeholder="부제목" value="${bookDto.sub_title }"/>
					</div>
					<div>
						<label>출간일</label>
						<div class="b_in_date">
						 	<input type="text" id="b_date" name="write_date" value="${bookDto.write_date}" placeholder="YY-MM-DD">
						</div>
					</div>
					<div>
						<label>출판사</label>
						<input type="text" name="publisher" placeholder="출판사" value="${publisherDto.name }"/>
					...
					<div>
						...
						<label id="keywordLabel"></label>
					</div>
					<div>
						<label>작가</label>
						<input type="text" name="author" value="${author.name}" placeholder="작가이름" oninput="checkAuthor('${root}','author')"/>
						<span class="b_in_check"></span>
					...
					<div>
						<label>일러스트</label>
						<input type="text" name="illustrator" value="${illustrator.name }" placeholder="일러스트이름" oninput="checkAuthor('${root}','illustrator')"/>
						<span class="b_in_check"></span>
						<div class="b_in_author_list">
							<ul></ul>
						</div>
					...
					<div>
						<label>번역가</label>
						<input type="text" name="translator" value="${translator.name }" placeholder="번역이름" oninput="checkAuthor('${root}','translator')"/>
						<span class="b_in_check"></span>
						<div class="b_in_author_list">
							<ul></ul>
						</div>
					...
					<div>
						<label>도서가격</label><input type="text" value="${bookDto.price }" name="price" placeholder="가격"/>
					</div>
					...
					<div class="b_in_img">
						<label>도서 이미지</label>
						<div>
							<img src="${bookDto.img_path }">
							<button id="b_in_filebtn" type="button" class="bf-button">찾기</button>
						</div>
					</div>
					<div class="b_in_textarea">
						<label>도서 소개</label>
						<textarea name="intro" rows="" cols="">${bookDto.intro }</textarea>
					</div>
					<div class="b_in_textarea">
						<label>출판사 서평</label>
						<textarea name="pub_intro" rows="" cols="">${bookDto.pub_intro }</textarea>
					</div>
					<div align="right">
					...
					<input type="hidden" name="pub_num" value="${bookDto.pub_num }">
					<input type="hidden" name="cate1_num">
					<input type="hidden" name="cate2_num">
					<input type="hidden" name="cate3_num">
					<input type="hidden" name="support" value="${bookDto.support }">
					<input type="hidden" name="img_path" value="${bookDto.img_path }">
					<input type="hidden" name="rental_period" value="${bookDto.rental_period}">
					<input type="hidden" name="rental_price" value="${bookDto.rental_price }">
					<input type="hidden" name="type" value="${bookDto.type }">
	...
	<script src="${root}/script/manager/book/update.js"></script>
	<script>
