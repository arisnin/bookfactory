...
function headerIndexInit(root) {
	...
	var generalList = document.querySelectorAll(".hw_menu_general .hw_general_sub");
	var count = 0;
	Array.prototype.forEach.call(generalList, function(e,i) {
		// 중뷴류 href 설정
		e.firstElementChild.setAttribute("href", root + "/category.do?cnum=" + thirdCategorySet[count]);
		// 소분류 메뉴 추가
		insertGeneralMenu(e.lastElementChild, count, sourceList[i]);
	...
	var etcList = document.querySelectorAll(".hw_menu_etc > .hw_box > span");
	Array.prototype.forEach.call(etcList, function(e,i) {
		// 중뷴류 href 설정
		e.firstElementChild.setAttribute("href", root + "/category.do?cnum=" + thirdCategorySet[count]);
		// 소분류 메뉴 추가
		insertEtcMenu(e.nextElementSibling, count, sourceList[i]);
		count += sourceList[i].length;
	});
...
function insertGeneralMenu(target, count, source) {
	source.forEach(function (e,i) {
		...
		$(a).addClass('hw_sub_category_link').attr("href", root_context + "/category.do?cnum=" + thirdCategorySet[count]).attr("title", thirdCategorySet[count++]).text(e);
		$(li).append(a);
		$(target).append(li);
	});
}
...
function insertEtcMenu(target, count, source) {
	source.forEach(function (e,i) {
		...
		$(a).addClass('hw_menu_etc_cate').attr("href", root_context + "/category.do?cnum=" + thirdCategorySet[count]).attr("title", thirdCategorySet[count++]).text(e);
		$(li).append(a);
		$(target).append(li);
	});
}
